  ...
  return (
    <main className="max-w-6xl mx-auto px-6 py-16">
      <section className="mb-16">
        ...
        <p className="mt-5 text-lg text-zinc-600 max-w-2xl">A personal space for writing, experiments and artwork.</p>
      </section>
      ...
      <section>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {(articles ?? []).map((a) => <Link key={a.slug} href={`/article/${a.slug}`} className="border rounded-xl p-5 bg-white hover:shadow">
            <p className="text-xs text-zinc-500">{a.category}</p><h3 className="font-bold text-xl mt-2">{a.title}</h3><p className="text-zinc-600 mt-2">{a.excerpt}</p>
          </Link>)}
        </div>
      </section>
